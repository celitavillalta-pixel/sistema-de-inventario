...
def registrar_producto(ventana, logo_label, boton_1, boton_2, boton_3, boton_editar=None):
    ...
    def volver_pantalla_principal():
        """Destruir tarjeta de registro y mostrar pantalla principal nuevamente"""
        tarjeta_registro.destroy()
        logo_label.pack(pady=20, padx=20)
        boton_1.pack(pady=20)
        boton_2.pack(pady=20)
        if boton_editar:
            boton_editar.pack(pady=20)
    ...
    boton_volver = ctk.CTkButton(master=tarjeta_registro, text="Volver", bg_color="#E0E0E0", fg_color="#00BFFE", width=250, height=40, command=volver_pantalla_principal)
